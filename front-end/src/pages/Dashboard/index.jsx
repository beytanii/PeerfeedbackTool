...
function DashboardPage() {
  ...
  const fetchFormData = async (userData) => {
    ...
    try {
      const pendingResponse = await fetch(
        `https://api.peerfeedback.betbet.website/forms/user/${userData.UserID}`
      );
      ...
      const createdResponse = await fetch(
        `https://api.peerfeedback.betbet.website/forms/creator/${userData.UserID}`
      );
  ...
  const handleDeleteForm = async (formID) => {
    if (window.confirm('Are you sure you want to delete this form?')) {
      try {
        const response = await fetch(`https://api.peerfeedback.betbet.website/forms/delete`, {
          method: 'DELETE',
          headers: {
