...
function FormCard({ formData, isMyForm, isCompleted, onDelete, currentUserID }) {
  ...
  useEffect(() => {
    const fetchGroupData = async () => {
      try {
        const response = await fetch(`https://api.peerfeedback.betbet.website/groups/${formID}`);
    ...
    const fetchCreatorPermissions = async () => {
      ...
      try {
        const response = await fetch(
          `https://api.peerfeedback.betbet.website/users/getCreatorPermissions?CreatorID=${currentUserID}&GroupID=${groupID}`
        );
        if (!response.ok) {
          throw new Error(
